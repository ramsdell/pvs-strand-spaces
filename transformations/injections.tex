...
\ref{item: uniqs}, and~\ref{item: origs} ensure the truths of \fn{non},
\fn{uniq}, and \fn{orig} are preserved.
...
\item\label{item: uniqs} Unique origination predicate respects injection.
  Let~$\atm$ be the atoms that occur in~$\bun_0$.
  \[\all{t\in\atm}\fn{uniq}(\ssp_1, t)\supset\fn{uniq}(\ssp_0, t)\]

\item\label{item: origs} Unique origination at predicate respects injection.
  \[\all{t\in\atm,n\in\nodes(\ssp_0)}\fn{orig}(\ssp_1,
  t,\ell(n))\supset\fn{orig}(\ssp_0, t,n)\]
\end{enumerate}
