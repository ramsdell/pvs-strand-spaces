...
\item $\prefix{\Theta(s)}{h}=\sigma[\alpha(z)/x]
  \circ\prefix{r}{h}$ for some~$\sigma$.
\end{enumerate}
where \[\sigma[y/x](z)=
\left\{
\begin{array}{ll}
...
\item Case of $P_1[g^r(r),h',g^\gamma(r)(x)](z,t)$ in~$T_g(\Psi)$ with
  ...
  \item
    $\prefix{\ssp_0(s)}{h}=\sigma_0[\alpha_0(t)/x]\circ
    ...
    $(\sigma_0[\alpha(t)/x])(x)=\alpha(t)$, then
    $(\sigma\circ\gamma)(x) =\sigma(\gamma(x))=\alpha(t)$.  Therefore
    $\prefix{\ssp_1(h^s(s))}{h'}=\sigma[\alpha(t)/\gamma(x)]\circ
    \prefix{c_1}{h'}$.  \remark{This must be wrong because of
      branching!}
  \end{itemize}
...
\item Case of $P_0[r,h,x](z,t)$ in~$\Psi$.  Assume $\bun_1,\alpha_1\models
  ...
  \item
    ...
    \sigma_1[\alpha_0(t)/\gamma(x)]\circ
    ...
    $\sigma=\sigma[\alpha_0(t)/\gamma(x)]$.  By
    Property~\ref{item: role mappings},
    $\prefix{\ssp_0(s)}{h}=\sigma\circ\gamma\circ\prefix{\role^c(r)}{h}$,
    and thus
    $\prefix{\ssp_0(s)}{h}=(\sigma\circ\gamma)[\alpha_0(t)/x]\circ
    \prefix{\role^{c}(r)}{h}$.
  \end{itemize}
...
formula, and~$Y$ is the formula's declarations, but first we define
the relevant nodes of a skeleton~$N$.  Let
$k=\skel_X(P,\insts,\prec,\nu,\upsilon)$ and let~$\prec^-$ be the
transitive reduction of~$\prec$.  The \emph{relevant nodes} of~$k$ are
$N=N_s\cup N_\prec\cup N_\upsilon$ where

\[\renewcommand{\arraystretch}{1.5}
\begin{array}{r@{{}={}}l}
  N_s&\{(s,i)\mid s\in\strands(\insts^\ssp)\land
  i=|\insts^\ssp|-1\}\\
  ...
  N_\upsilon&\{(s,i)\mid\some{t\in\upsilon,(s,i)\in
    \nodes(\insts^\ssp)}(s,i)\in\orig_k(t)\}
\end{array}\]

For~$\form(k)=(Y,\Phi)$, the declarations~$Y$ is~$X$ augmented with a
fresh variable of sort~\srt{N} for each node in~$N$, and let $v(n)$ be
...
\item For each $s\in\sdom(I)$, let $I(s)=\insta(r,h,\sigma)$.  For
  ...
  $P[r,x](v(s,h-1),t)$.
\item For each $(s,i),(s,i')\in V$ such that $i<i'$, assert
  $\cn{coll}(v(s,i),v(s,i'))$.
...
of~$\sigma_i$ and the term it maps to.  Map~$\phi_i$ is coded as
equalities between strand variables in~$\Phi_0$ and strand variables
in~$\Phi_i$.  Let~$v_0$ be the node variables freshly
generated for~$k_0$, and~$v_i$ be the ones generated for~$k_i$.  The
strand mapping part of~$\Delta_i$ is
...
\cn{prec}(x,z).\]
Furthermore, $\cn{coll}(x,y)\supset\cn{prec}(x,y)$.

\paragraph{Protocol Transformations and Formula Translations.}

Function~$f$ is \emph{increasing} iff $i<j$ implies $f(i)<f(j)$.  An
increasing function is injective.

A \emph{trace stretch} from trace~$c_0$ to~$c_1$ is an increasing
function $f\in\sdom(c_0)\to\sdom(c_1)$ such that for all~$i<|c_0|$,
$c_0(i)$ is inbound iff $c_1(f(i))$ is inbound.

Suppose~$g$ maps each role $r_0\in P_0$ to a triple $(r_1,f,\gamma)$,
where $r_1\in P_1$, $f$ is a trace stretch from the trace of~$r_0$ to
the trace of~$r_1$, and~$\gamma\in\vars(c_0)\to\vars(c_1)$.
Function~$g$ is a \emph{protocol injection} iff
\begin{enumerate}
\item $\gamma$ is injective, and
\item $g(r_0)=(r_1,f,\gamma)$ and
$g(r'_0) = (r_1,f',\gamma')$ implies $r_0=r'_0$.
\end{enumerate}
Every protocol transformation defines a protocol injection.  The role
map of a protocol injection~$g$ is written~$g^r$, the role trace
stretch is written~$g^f$, and the role variable renaming
is~$g^\gamma$, so $g(r_0)=(g^r(r_0), g^f(r_0),g^\gamma(r_0))$.

Let $\Psi$ be a security goal in~$\lang(P_0)$ and $g$ be a protocol
injection.  To import~$\Psi$ into~$\lang(P_1)$, translate each atomic
formula in~$\Psi$ as follows.
\begin{itemize}
\item $P_0[r,i](v)$ becomes $P_1[g^r(r), g^f(i)](v)$, 
\item $P_0[r,x](v, t)$ becomes
  $P_1[g^r(r),g^\gamma(r)(x)](v,t)$,
\item Otherwise, the atomic formula is not changed.
\end{itemize}
