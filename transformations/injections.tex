...
the trace of~$r_0$ to the trace of~$r_1$.  The role map of a protocol
injection~$g$ is written~$g^r$ and the role to trace stretch is
written~$g^f$ so $g(r_0)=(g^r(r_0), g^f(r_0))$.
...
\item for all bundles~$\bun_1$ that are runs of~$P_1$ with role
  ...
  \item node stretch
    ...
    |\ssp_1(h^s(s))|]$,
  \end{itemize}
  where $\ssp_0=\bun^\ssp_0$ and $\ssp_1=\bun^\ssp_1$, the following
  conditions hold.
\end{itemize}
\begin{enumerate}
\item Roles respect injection (for role predicate mappings)
  \[\begin{array}{l}
