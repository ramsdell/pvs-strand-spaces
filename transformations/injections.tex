...
is called a \emph{non-atomic variable}.  We write $t\typ S$ to say
that term~$t$ is in the carrier set of sort~$S$.
...
$\sigma\circ\prefix{r}{h}$, the prefix of length~$h$ that results from
applying~$\sigma$ to~$r$.

A sequence of instances~$\insts$ defines a strand space~{\ssp} and a
role assignment~\rl, where $\sdom(\insts)=\strands(\ssp)$ and when
$\insts(s)=\insta(r,h,\sigma)$, $\ssp(s)=\sigma\circ\prefix{r}{h}$ and
$\rl(s)=r$.  Let $\insts^\ssp$ and $\insts^{\rl}$ be the strand space
and role assignment associated with~$\insts$.

$\skel(P,\insts,\prec,\nu,\upsilon)$

In the {\cpsa} syntax, the role map and sequence of traces are
replaced by a sequence of instances.  So for skeleton
$\skel_X(\rl,P,\Theta_X,\prec,N,U)$, the {\cpsa} syntax is
$\skel_X(P,I,\prec,N,U)$, where for each $s\in\sdom(\Theta_X)$,
$I(s)=\insta(r,h,\sigma)$, $r=\rl(s)$, and the trace of
$\insta(r,h,\sigma)$ is $\Theta_X(s)$.

\paragraph{Homomorphisms.}

Let $k_0=\skel_X(rl_0,P,\Theta_0,\prec_0,N_0,U_0)$ and
$k_1=\skel_Y(rl_1,P,\Theta_1,\prec_1,N_1,U_1)$ be skeletons.  There is
a \emph{skeleton homomorphism} $(\phi,\sigma)\colon k_0\mapsto k_1$
...
\item\label{item:orig} $t\in U_0$ implies
  $\phi(\orig_{k_0}(t))\subseteq\orig_{k_1}(\sigma(t))$;
\end{enumerate}
where $\orig_k(t)$ is the set of nodes of events at which~$t$
originates.  Item~\ref{item:orig} says the node at which an atom
declared to be uniquely originating is preserved by homomorphisms.
Note that~$\phi$ is a strand mapping, not a node mapping as
in~\cite{Guttman09}.
...
\land\sigma\circ\prefix{\role^c(r)}{h}=\ssp(s).\]
